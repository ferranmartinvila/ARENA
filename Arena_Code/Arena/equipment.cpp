...
//CONSTRUCTOR----------------
equip::equip(char* name, char* description, OBJECT_TYPE object_type, uint live_buff = 0, uint defence_buff = 0, uint attack_buff = 0, uint agility_buff = 0, uint price = 0) :object(name, description, object_type, price), live_buff(live_buff), defence_buff(defence_buff), attack_buff(attack_buff), agility_buff(agility_buff),level(1){}

//LORE-----------------------
void equip::look_it()const{
	slim_printf(WHITE, "%s [%u]: %s\n\nBUFFS:\n", name.get_string(), level, description.get_string());
...
void equip::pauted_look_it()const{
	...
	printf(" -> ");
	slim_printf(YELLOW, " lvl: %u ", level);
